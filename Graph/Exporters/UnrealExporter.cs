...
{
    public class UnrealExporter : Exporter
    {
        string path;
        Graph graph;
        int index;
        ...
        public UnrealExporter(Graph g, string exportPath)
        {
            if (g == null) return;
            ...
            for (int i = 0; i < g.OutputNodes.Count; ++i)
            {
                ...
                if (g.NodeLookup.TryGetValue(id, out Node n))
                {
                    ...
                    if (on.OutType == OutputType.metallic || on.OutType == OutputType.roughness
                        || on.OutType == OutputType.occlusion || on.OutType == OutputType.height)
                    {
            ...
            if (graph.NodeLookup.TryGetValue(nid, out n))
            {
                ...
                switch (on.OutType)
                {
                    case OutputType.metallic:
                        ...
                        {
                            merge.CopyRedToBlue(bmp);
                        }
                        break;
                    case OutputType.roughness:
                        if (merge.Width >= bmp.Width && merge.Height >= bmp.Height)
                        {
                            merge.CopyRedToGreen(bmp);
                        }
                        break;
        ...
        public override void Complete()
        {
            ...
            if (merge.Width > 0 && merge.Height > 0)
            {
                ...
                using (FileStream fs = new FileStream(System.IO.Path.Combine(path, name + "_occroughmetalheight.png"), FileMode.OpenOrCreate))
                {
                    src.Save(fs, System.Drawing.Imaging.ImageFormat.Png);
                }
